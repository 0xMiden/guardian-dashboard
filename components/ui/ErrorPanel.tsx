 ...
 *
 * The branch is on the Guardian's `code` first and the HTTP status second, which is
...
function describe(error: FetchError): ErrorShape {
  ...
  if (code === "insufficient_operator_permission" || error.status === 403) {
    ...
    return {
      ...
      detail: missing
        ? `This Guardian has not granted the dashboard's operator key ${missing}. Ask whoever runs it to add the permission to their allowlist entry.`
        : "This Guardian has not granted the dashboard's operator key permission for this action.",
      retryable: false,
    };
  }

  // The proxy re-authenticates once before giving up, so a 401 reaching here
  // means the Guardian does not accept our key at all rather than that a session
  // aged out. That is the state a newly-added Guardian sits in until it allowlists
  // the key, and it is not something the reader can fix by signing in again.
  if (code === "authentication_failed" || error.status === 401) {
    return {
      icon: <Lock className="h-4 w-4" />,
      title: "Guardian does not recognise this operator key",
  ...
  if (error.status === 429) {
    return {
      ...
      detail: retryAfterSecs
        ? `The Guardian asked us to wait ${retryAfterSecs}s before trying again.`
        : "The Guardian is rate-limiting this operator. Wait a moment before trying again.",
      retryable: true,
    };
  }
